...
public class JPopup extends Window {
	...
	public JPopup(int par0index) {
		...
		setBounds(EnumData.POPUP_LOC_X.getInt(), EnumData.POPUP_LOC_Y.getInt(),
				EnumData.POPUP_WIDTH.getInt(), EnumData.POPUP_HEIGHT.getInt());
		setFocusable(false);
		...
		labelIconNormal.setBounds(1, 0, EnumData.ICON_SIZE_NORMAL.getInt(),
		...
		labelIconPin.setBounds(EnumData.POPUP_WIDTH.getInt()
				- (EnumData.ICON_SIZE_PIN.getInt() + 3), 3,
				EnumData.ICON_SIZE_PIN.getInt(),
				EnumData.ICON_SIZE_PIN.getInt());
		labelIconPin.addMouseListener(new NTMouseListener(index));
		labelIconPin.setIcon(NotifyTweet.getInstance().getResourceManager()
				.getIconPinOff());

		labelTextUser = new JLabel("User");
		labelTextUser.setBounds(55, 0, EnumData.POPUP_WIDTH.getInt() - 70, 20);
		labelTextUser.setFont(NotifyTweet.getInstance().getConfigManager()
				.getFontName());

		labelTextText = new JLabel("Text");
		labelTextText.setBounds(55, 24, 290, 20);
		labelTextText.setFont(NotifyTweet.getInstance().getConfigManager()
				.getFontText());
	...
	/**
	 * ピンの状態を変更します. ピンを指す場合はtrue,抜く場合はfalseを渡します.
	 * 
	 * @param ispin
	 *            boolean:ピンを有効にする
	 */
